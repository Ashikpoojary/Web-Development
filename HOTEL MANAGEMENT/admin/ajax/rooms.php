...
if(isset($_POST['add_room'])){
    ...
    if(insert_query($q1,$values,'siiiiis')){
        $flag = 1;
    }
...
if(isset($_POST['get_all_rooms'])){
    $res = select_query("SELECT * FROM `rooms` WHERE `removed`=?",[0],'i');
    $i = 1;
...
if(isset($_POST['get_room'])){
    ...
    $res1 = select_query("SELECT * FROM `rooms` WHERE `id`=?",[$frm_data['get_room']],'i');
    $res2 = select_query("SELECT * FROM `rooms_features` WHERE `room_id`=?",[$frm_data['get_room']],'i');
    $res3 = select_query("SELECT * FROM `rooms_facilities` WHERE `room_id`=?",[$frm_data['get_room']],'i');
...
if(isset($_POST['edit_room'])){
    ...
    if(update_query($q1,$values,'siiiiisi')){
        $flag = 1;
    }

    $del_features = delete_query("DELETE FROM `rooms_features` WHERE `room_id`=?",[$frm_data['room_id']],'i');
    $del_facilities = delete_query("DELETE FROM `rooms_facilities` WHERE `room_id`=?",[$frm_data['room_id']],'i');
...
if(isset($_POST['get_room_images'])){
    $frm_data = filteration($_POST);
    $res = select_query("SELECT * FROM `room_images` WHERE `room_id`=?",[$frm_data['get_room_images']],'i');
...
if(isset($_POST['remove_room'])){
    ...
    $res1 = select_query("SELECT * FROM `room_images` WHERE `room_id`=?",[$frm_data['room_id']],'i');

    while($row = mysqli_fetch_assoc($res1)){
        deleteImage($row['image'],ROOMS_FOLDER);
    }

    $res2 = delete_query("DELETE FROM `room_images` WHERE `room_id`=?",[$frm_data['room_id']],'i');
    $res3 = delete_query("DELETE FROM `rooms_features` WHERE `room_id`=?",[$frm_data['room_id']],'i');
    $res4 = delete_query("DELETE FROM `rooms_facilities` WHERE `room_id`=?",[$frm_data['room_id']],'i');
    $res5 = delete_query("UPDATE `rooms` SET `removed`=? WHERE `id`=?",[1,$frm_data['room_id']],'ii');
